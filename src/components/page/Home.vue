...
const fetchPosts = async () => {
  ...
  // 정렬 기준이 존재한다면
  if (orderStandard !== null || orderStandard !== 'undefined') {
    param.sort = orderStandard.condition + ',DESC';
  } else {
  ...
  for (let i = 0; i < filters.length; i++) {
    ...
    if (filter.searchGroup === 'category') {
      param.category = param.category === undefined ? filter.searchValue : param.category + ',' + filter.searchValue;
    } else if (filter.searchGroup === 'status') {
...
const setOrderStandard = (_orderStandard) => {
  orderStandard.label = _orderStandard.label;
  orderStandard.condition = _orderStandard.condition;
}
</script>
